...
func (l *loggerUtil) Attrs(c Ctx, attrss ...[]any) []any {
	...
	attrs = append(attrs, slog.String("path", c.Request.Path))
	attrs = append(attrs, slog.String("referer", c.Request.Referer))
	if c.Err != nil {
		attrs = append(attrs, slog.String("err_message", Error().GetOriginalMessage(c.Err)+"\n"))
		detailByte, _ := json.MarshalIndent(Error().Detail(c.Err), "", "  ")
		if detail := string(detailByte); detail != "" && detail != "null" {
			attrs = append(attrs, slog.String("err_detail", detail+"\n"))
		}
		traceByte, _ := json.MarshalIndent(Error().TraceSimple(c.Err), "", "  ")
...
func (l *loggerUtil) sendAlert(msg string, args []any) {
	...
	for _, a := range args {
		if attr, ok := a.(slog.Attr); ok {
			if e, ok := attr.Value.Any().(error); ok {
				msg = Error().GetOriginalMessage(e)
				detailByte, _ := json.MarshalIndent(Error().Detail(e), "", "  ")
				detail = string(detailByte)
				traceByte, _ := json.MarshalIndent(Error().TraceSimple(e), "", "  ")
	...
	writeBuff("```")
	writeBuff(msg)
	acceptedKey := []string{"env", "version", "method", "path", "referer"}
	for _, a := range args {
		if attr, ok := a.(slog.Attr); ok && slices.Contains(acceptedKey, attr.Key) {
			writeBuff(attr.Key, ": ", attr.Value.String())
		}
	}
	if detail != "" && detail != "null" {
		writeBuff("error: ", detail)
	}
	if trace != "" && trace != "null" {
		writeBuff("trace: ", trace)
	}
	writeBuff("```")
